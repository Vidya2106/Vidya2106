# Hi there 👋, I’m Vidya Sri

🎓 **B.Sc. Computer Science Student | 2023–2026**  
📊 Aspiring Data Analyst & Data Scientist

---

## About Me
I am a curious and motivated CS student with a passion for turning data into meaningful insights.  
I enjoy working on small projects to learn new tools, understand business problems, and apply practical solutions.

🌱 Currently Learning:
- Advanced Data Analytics
- AI & ML
- Power BI

---

## Skills

### Programming Languages
- Python 🐍   
- SQL 
- Power BI / Tableau  

### Data Analytics & Visualization
- Power BI / Tableau 
- Data Cleaning & Transformation  
- Data Visualization & Dashboarding  
- Basic Statistics & Exploratory Data Analysis  

### Other Tools
- Git & GitHub  
- Excel  
- Jupyter Notebook / VS Code  

---

## Projects

### 💾 Hospital Patient Management System – SQL
- Designed and implemented a database to manage hospital patient records
- Created tables for patients, doctors, appointments, and treatments
- Wrote queries to retrieve patient history, doctor schedules, and billing information
- Learned database relationships, joins, and aggregations

### 📊 Student Social Media Addiction Analysis – Excel
- Analyzed survey data of students using Excel
- Created pivot tables, charts, and graphs to visualize patterns
- Derived insights on social media usage trends and their impact on academics

### 🐍 Personal Finance Manager with AI Assistance – Python
- Built a Python application to track personal expenses and income
- Implemented AI-assisted recommendations for budgeting and saving
- Used pandas, matplotlib, and basic AI logic to provide actionable insights

### 📈 Bank Customer Prediction – Python (EDA)
- Explored and cleaned bank customer dataset for analysis
- Performed statistical analysis and visualizations to identify patterns
- Analyzed factors affecting customer behavior and potential predictions

---

## 💼 Internships & Experience

**Online Internship – Data Science and Data Analytics - Edify Techno Solutions**
- Learned basic data cleaning, visualization, and exploratory analysis
- Gained understanding of real-world data-driven decision making

---

🏆 Certifications:  
- AWS Academy Graduate – AWS Academy Cloud Foundations 
- Deloitte Virtual Internship – Data Analytics Job Simulation (Forage)
- Cloud Infrastructures & Services – Hoping Minds

---

## Connect with me
- ✉️ Email: vidyasridevendran@gmail.com  
- 🔗 LinkedIn: [linkedin.com/in/vidya-sri-37394a35b](https://www.linkedin.com/in/vidya-sri-37394a35b)  

---


